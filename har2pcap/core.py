...
def hmm():
    ...
    target = open('test.pcapng', 'wb')

    # write wiresharks special magic header, how knows what it stands for
    target.write(bytes.fromhex('0a 0d 0d 0a 1c 00 00 00 4d 3c 2b 1a 01 00 00 00'))
    target.write(bytes.fromhex('ff ff ff ff ff ff ff ff 1c 00 00 00 01 00 00 00'))
    target.write(bytes.fromhex('48 00 00 00 01 00 00 00 00 00 04 00 02 00 07 00'))
    target.write(bytes.fromhex('65 6e 70 30 73 32 35 00 09 00 01 00 09 00 00 00'))
    target.write(bytes.fromhex('0c 00 16 00 4c 69 6e 75 78 20 34 2e 34 2e 30 2d'))
    target.write(bytes.fromhex('32 32 2d 67 65 6e 65 72 69 63 00 00 00 00 00 00'))
    target.write(bytes.fromhex('48 00 00 00 06 00 00 00 08 02 00 00 00 00 00 00'))
    target.write(bytes.fromhex('ef be 53 14 10 c9 08 1b  e6 01 00 00 e6 01 00 00'))
    ...
    target.write(bytes.fromhex('00 00 08 02 00 00'))
